...
import com.shooping.ecommerce.service.entity.Pedido;
...
import com.shooping.ecommerce.service.repository.OrderRepository;
...
public class OrderService {
  ...
  private final MenuRepository pRepository;

  public OrderService(OrderRepository repository, OrderDetailRepository orderDetailRepository, DetallePedidoService dpService,
                      MenuRepository pRepository) {
  ...
  public GenericResponse<List<PedidoConDetallesDTO>> devolverMisCompras(int idCli) {
    ...
    pedidos.forEach(p -> {
      dtos.add(new PedidoConDetallesDTO(p, orderDetailRepository.findByPedido(p.getId())));
    });
    return new GenericResponse(Global.OPERACION_CORRECTA, Global.RPTA_OK, "Petición Encontrada", dtos);
  }
  ...
  public GenericResponse guardarPedido(GenerarPedidoDTO dto) {
    ...
    return new GenericResponse(Global.TIPO_DATA, Global.RPTA_OK, Global.OPERACION_CORRECTA, dto);
  ...
  public GenericResponse anularPedido(int id) {
    ...
    if (p.getId() != 0) {
      ...
      return new GenericResponse(Global.TIPO_DATA, Global.RPTA_OK, Global.OPERACION_CORRECTA, p);
    } else {
      return new GenericResponse(Global.TIPO_DATA, Global.RPTA_OK, Global.OPERACION_ERRONEA, "El pedido que desea anular no es válido");
    }
  }

  private void restablecerStock(final int pedidoId) {
    Iterable<DetallePedido> detalles = this.orderDetailRepository.findByPedido(pedidoId);
  ...
  public ResponseEntity<Resource> exportInvoice(int idCli, int idOrden) {
    ...
    Double rpta = this.orderDetailRepository.totalByIdCustomer(idCli);
    if (optPedido.isPresent()) {
      try {
        ...
        parameters.put("dsInvoice", new JRBeanCollectionDataSource((Collection<?>) this.orderDetailRepository.findByPedido(idOrden)));
